...
public class OperateView extends View
{
	...
	private float picScale = 1f; //Changed by Mostofa
...
//						}
//						if (getSelected().isTextObject) {
//							myListener.onClick((TextObject) getSelected()); //Changed by Mostofa!
//						} else {
...
//						}
				...
				for (int i = 0; i < imgLists.size(); ++i)
				{
					...
					if (i != selectedId)
					{
						...
						if (getSelected() == null) {
							if (io.isTextObject) {
								myListener.onOutSideClick((TextObject) io);//Changed by Mostofa!
							} else {
			...
			case MotionEvent.ACTION_UP :
				...
				if (getSelected() != null) {
					if (getSelected().isTextObject) {
						myListener.onClick((TextObject) getSelected(), isMoveEvnt); //Changed by Mostofa!
					} else {
						myListener.onImageObjectClick(getSelected());
					}
				}
	...
	* */
	...
	public interface MyListener
	{
		...
		public void onOutSideClick(TextObject textObject);//Changed by Mostofa!
