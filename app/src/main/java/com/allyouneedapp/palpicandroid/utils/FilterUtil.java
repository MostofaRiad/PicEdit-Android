...
public class FilterUtil {
    ...
    public static GPUImageFilter createFilterForType(final Context context, final FilterType type) {
        switch (type) {
            ...
            case LOOKUP_AMATORKA:
                GPUImageLookupFilter amatorka = new GPUImageLookupFilter(0.5f);
            ...
            case HAZE:
                GPUImageFilterGroup haze = new GPUImageFilterGroup();
                GPUImageLookupFilter amatork = new GPUImageLookupFilter(0.5f);
                amatork.setBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.lookup_amatorka));
                haze.addFilter(amatork);
                haze.addFilter(new GPUImageRGBFilter(1.3f, 1.1f, 0.9f));
                return haze;
            case LAPLACIAN:
            ...
            case NEWRGB:
                GPUImageFilterGroup newRGB = new GPUImageFilterGroup();
                newRGB.addFilter(new GPUImageRGBFilter(1.3f, 1.1f, 0.9f));
                GPUImageToneCurveFilter toneCurve = new GPUImageToneCurveFilter();
                toneCurve.setFromCurveFileInputStream(
                        context.getResources().openRawResource(R.raw.tone_cuver_sample));
                newRGB.addFilter(toneCurve);
                return newRGB;
            case LIGHTGRAY:
                GPUImageLevelsFilter levels = new GPUImageLevelsFilter();
                levels.setMin(0.0f, 3.0f, 1.0f);
                GPUImageFilterGroup lightGray = new GPUImageFilterGroup();
                lightGray.addFilter(levels);
                lightGray.addFilter(new GPUImageGrayscaleFilter());
                return lightGray;

            case PROCESS:

                GPUImageFilterGroup process = new GPUImageFilterGroup();
                process.addFilter(new GPUImageHueFilter(90.0f));
                process.addFilter(new GPUImageRGBFilter(1.3f, 1.1f, 0.9f));
                return  process;
            case BRIGHT:
                GPUImageFilterGroup bright = new GPUImageFilterGroup();
                bright.addFilter(new GPUImageRGBFilter(1.3f, 1.1f, 0.9f));
                GPUImageToneCurveFilter toneC = new GPUImageToneCurveFilter();
                toneC.setFromCurveFileInputStream(
                        context.getResources().openRawResource(R.raw.tone_cuver_sample));
                bright.addFilter(toneC);
                return bright;
            case NEWSEPIA:
                GPUImageFilterGroup sepia = new GPUImageFilterGroup();
                sepia.addFilter(new GPUImageSepiaFilter());
                sepia.addFilter(new GPUImageRGBFilter(1.3f, 1.1f, 0.9f));
                GPUImageToneCurveFilter toneCu = new GPUImageToneCurveFilter();
                toneCu.setFromCurveFileInputStream(
                        context.getResources().openRawResource(R.raw.tone_cuver_sample));
                sepia.addFilter(toneCu);
                return sepia;
            case ORIGINAL:
    ...
    public enum FilterType {
        ...
        SPHERE_REFRACTION, SWIRL, WEAK_PIXEL_INCLUSION, FALSE_COLOR, COLOR_BALANCE, LEVELS_FILTER_MIN, BILATERAL_BLUR, HALFTONE, TRANSFORM2D,NEWRGB,LIGHTGRAY,
        INSTANT,TRANSFER,FADE,VIVID,PROCESS, BRIGHT, NEWSEPIA, MONO, LOOKUP, HAZERGBLOOKUP,ORIGINAL
    }
